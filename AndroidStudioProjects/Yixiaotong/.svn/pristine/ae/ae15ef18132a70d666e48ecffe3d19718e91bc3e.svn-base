...
public class FragmentTabUtil implements RadioGroup.OnCheckedChangeListener {
    ...
    public FragmentTabUtil(FragmentManager fm, List<Fragment> fragmentList,
                           int contentId, RadioGroup radioGroup,RadioGroupCheckChanged checkChanged) {
        this.fm = fm;
        this.fragmentList = fragmentList;
        this.contentId = contentId;
        this.radioGroup = radioGroup;
        this.mRadioGroupCheckChanged = checkChanged;
...
//        }
    ...
    public void onCheckedChanged(RadioGroup radioGroup, int id) {
        for (int j = 0; j < radioGroup.getChildCount(); j++) {
            if(radioGroup.getChildAt(j).getId() == id){
                displayFragment(j);
                mRadioGroupCheckChanged.onRgCheckChanged(j);
            }
        }
    }
    ...
    public interface RadioGroupCheckChanged{
        void onRgCheckChanged(int position);
    }
}
