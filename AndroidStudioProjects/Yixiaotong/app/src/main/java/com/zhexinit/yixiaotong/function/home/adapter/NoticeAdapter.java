...
public class NoticeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof NoticeCommonHolder) {
            ...
            commonHolder.itemTvClass.setText(noticeResp.schoolName  + noticeResp.classOtherName+" "+noticeResp.childName);
            Glide.with(mContext)
            ...
            Glide.with(mContext)
                    .load(mRespList.get(position).messageIcon)
                    .diskCacheStrategy(DiskCacheStrategy.ALL)
                    .error(noticeResp.messageType==8?R.mipmap.ic_notice_power:R.mipmap.ic_notice_warn)
                    .transform(new GlideCricleTransform(mContext))
                    .into(commonHolder.itemIvImage);
