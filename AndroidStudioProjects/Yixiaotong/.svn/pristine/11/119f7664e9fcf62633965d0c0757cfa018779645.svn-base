...
public class HttpManager extends BaseHttpManager {
        ...
        Observable.just(request)
                ...
                .flatMap(new Function<BaseRequest, ObservableSource<Object>>() {
                    ...
                    public ObservableSource<Object> apply(@NonNull BaseRequest baseRequest) throws Exception {
                        return mApi.post(path,header, Config.getIMEI(SmartCampusApp.context),RequestBody.create(MediaType.parse("application/json; charset=utf-8"),mGson.toJson(request)));
                    }
                })
                ...
                .subscribe(new Observer<T>() {
                    ...
                    public void onError(@NonNull Throwable e) {
                        ...
                        if ("HTTP 505 ".equals(e.getMessage())||"HTTP 506 ".equals(e.getMessage())){
                            isOutLogin();
                        }
                        if (e instanceof IOException) {
                            if (e instanceof SocketTimeoutException || e instanceof ConnectException || e instanceof UnknownHostException) {
                                callback.onFail("无法连接网络");
                            } else {
                                callback.onFail("未知错误");//e.toString()
                            }
                        } else {
                            callback.onFail("网络错误");//e.toString()
                        }
                    }
        ...
        Observable.just(data)
                ...
                .flatMap(new Function<String, ObservableSource<Object>>() {
                    ...
                    public ObservableSource<Object> apply(@NonNull String baseRequest) throws Exception {
                        return mApi.post(path,header, Config.getIMEI(SmartCampusApp.context),RequestBody.create(MediaType.parse("application/json; charset=utf-8"),data));
                    }
                })
                .map(new Function<Object, T>() {
                    @Override
    ...
    public <T> void postExecute(final String path, final String header, final ResultCallBack<T> callback) {
        ...
        Observable.just("")
                ...
                .flatMap(new Function<String, ObservableSource<Object>>() {
                    @Override
                    public ObservableSource<Object> apply(@NonNull String baseRequest) throws Exception {
                        return mApi.post(path,header,Config.getIMEI(SmartCampusApp.context));
                    }
                })
                .map(new Function<Object, T>() {
                    @Override
        ...
        Observable.just(map)
                ...
                .flatMap(new Function<Map, ObservableSource<Object>>() {
                    ...
                    public ObservableSource<Object> apply(@NonNull Map map) throws Exception {
                        return mApi.post(path, header, Config.getIMEI(SmartCampusApp.context),mapToRequestBody(map));
                    }
                })
                .map(new Function<Object, T>() {
                    @Override
                ...
                .subscribe(new Observer<T>() {
                    ...
                    @Override
                    public void onError(@NonNull Throwable e) {
                        Log.e(TAG, "onError:" + e.getMessage()+"ccc");
                        if ("HTTP 506 ".equals(e.getMessage())||"HTTP 505 ".equals(e.getMessage())){
                            isOutLogin();
                        }
                        if (e instanceof IOException) {
                            if (e instanceof SocketTimeoutException || e instanceof ConnectException || e instanceof UnknownHostException) {
        ...
        Observable.just("")
                ...
                .flatMap(new Function<String, ObservableSource<Object>>() {
                    ...
                    public ObservableSource<Object> apply(@NonNull String map) throws Exception {
                        return mApi.get(path, header,Config.getIMEI(SmartCampusApp.context));
                    }
                })
                .map(new Function<Object, T>() {
                    @Override
    ...
    public void download(final String url, final String dirName, final String fileName, final DownloadListener listener) {
        ...
        Log.e(TAG, "download: "+dirName +"     "+ fileName);
        Observable.just(url)
                .subscribeOn(Schedulers.io())
                ...
                subscribe(new Observer<File>() {
                    ...
                    @Override
                    public void onError(@NonNull Throwable e) {
                        listener.fail(e.getMessage(),dirName, fileName);
                    }
    ...
    private Iterable<Observable<Object>> getMultiRequest(List<MultiRequestModel> requestModels) {
        ...
        for (MultiRequestModel request : requestModels) {
            switch (request.mRequestType) {
                case Post:
                    observables.add(mApi.post(request.path,request.header,Config.getIMEI(SmartCampusApp.context), objToRequestBody(request.baseRequestModel)));
                    break;
                case Get:
                    observables.add(mApi.get(request.path,request.header,Config.getIMEI(SmartCampusApp.context)));
                    break;
            }
        }
        return observables;
    ...
    private void isOutLogin(){
        final CommonDialog dialog = new CommonDialog(BaseWarehouse.mContext, "该账号已在其他设备登录", "如果非本人操作，请尽快修改密码");
        dialog.setSumbitClick(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                BaseWarehouse.mContext.startActivity(new Intent(BaseWarehouse.mContext, LoginWithSMSActivity.class));
            }
        });
        dialog.setCancelable(false);
        dialog.setNagetiveGone();
        dialog.show();
    }
